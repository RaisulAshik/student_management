   ...
   <header class="page-header">
					...
					<section class="panel">
							<header class="panel-heading">
								...
								<a class="" href="{{ action('ExpenseCategoryController@create') }}"><i class="fa fa-plus"></i> Add
								</a>
								</div>
								...
								<table class="table table-bordered table-striped mb-none" id="datatable-default">
									...
									<tbody>
										...
										<tr class="gradeX">
											<td>{{$category->name}}</td>
											<td>
												{{($category->expense_head_id != null) ? $category->admin->name:'' }}
											</td>
											
											<td>
												<a href="{{ route('expenseCategory.edit',$category->id) }}" class="mb-xs mt-xs mr-xs btn btn-primary" >Edit</a>

												{{-- <form method="post" action="{{ route('expenseCategory.destroy',$branch->id) }}"  style="display: inline">
                                                @csrf
                                                @method('delete')
