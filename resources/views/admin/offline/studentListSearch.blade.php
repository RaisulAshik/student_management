   ...
   <header class="page-header">
                     ...
                     <section class="panel">
							...
							<div class="panel-body">
                                <form method="get" action="{{action('AdminOfflineStudentController@listSearch')}}">
			                    @csrf
			                        <div class="form-group">
										...
										<div class="col-md-6">
											<select class="form-control mb-md" name="student_type" id="student_type">
												<option value="0" {{ ($request_student_type == 0) ? 'selected="selected"' : '' }} >Offline</option>
												<option value="1" {{ ($request_student_type == 1) ? 'selected="selected"' : '' }} >Online</option>
									...
									<div class="form-group">
										...
										<div class="col-md-6">
											<select class="form-control mb-md" name="branch_id" id="branch">
												...
												@endif
												<input type="hidden" id="auth-user-id" value="{{ auth('admin')->id() }}">
											</select>
									...
									<div class="form-group">
										...
										<div class="col-md-6">
											<select class="form-control mb-md" name="batch_id" id="batch">
												...
												@endphp
												@if($request_batch)
													<option value="{{$batch->id}}" selected="true">{{$batch->name}}</option>
												@else
													<option value="0" selected="true" disabled="true">Select Batch</option>
												@endif
											</select>
					...
					<section class="panel">
							<header class="panel-heading">
								...
								<form method="get" action="{{ action('AdminStudentController@listSearchPdf') }}"  style="display: inline">
                                    @csrf
                                    <input type="hidden" name="request_student_type" value="{{$request_student_type}}">
                                    <input type="hidden" name="request_branch_id" value="{{$request_branch}}">
                                    <input type="hidden" name="request_class_id" value="{{$request_class}}">
                                    <input type="hidden" name="request_batch_id" value="{{$request_batch}}">
                                    <input type="hidden" name="request_subject_id" value="{{$request_subject}}">
								...
								<table class="table table-bordered table-striped mb-none" id="datatable-default">
									...
									<tbody>
										...
										{{--<td>
											...
											{{--<td>
												{{($student->admin_id != null) ? $student->admin->name:'' }}
											</td>--}}
											...
											<td>
												<a style="" href="{{ route('admin.student_view',$student->id) }}" class="mb-xs mt-xs mr-xs btn btn-sm btn-primary" >View</a>

												<a style="" href="{{ route('admin.edit_offline_student',$student->id) }}" class="mb-xs mt-xs mr-xs btn btn-sm btn-success" >Edit</a>

												<a style="" href="{{ route('admin.offline_payments',$student->id) }}" class="mb-xs mt-xs mr-xs btn btn-sm btn-danger" > Payments</a>

												<a href="{{ route('admin.student_change_due_date',$student->id) }}" class="mb-xs mt-xs mr-xs btn btn-sm btn-danger" >Change Due Date</a>
                                                
												<a href="{{ route('admin.add_offline_student_course',$student->id) }}" class="mb-xs mt-xs mr-xs btn btn-sm btn-danger" >Add Course</a>
												
												<a href="{{ route('admin.offline_student_registration_pdf',$student->id) }}" class="mb-xs mt-xs mr-xs btn btn-sm btn-danger" >Registration Pdf</a>
		
												<a href="{{ route('admin.individualStdExamResult',$student->registration_id) }}" class="mb-xs mt-xs mr-xs btn btn-sm btn-danger" >View All Results</a>
