    ...
    public function listSearch(Request $request)
{
    ...
    $request_subject = $request->subject_id;
    ...
    return view('admin.offline.studentListSearch', compact('students', 'request_student_type', 'request_branch', 'request_class', 'request_batch', 'request_subject'));
}
    ...
    public function listSearchPdf(Request $request)
    {
        ...
        $pdf = PDF::loadView('admin.offline.studentPdf', compact('students', 'request_student_type', 'request_branch', 'request_class', 'request_batch', 'request_subject'));
    ...
    public function create_student(Request $request)
    {
        ...
        $number = $request->phone;
        $message ='Registration ID:'.$registration_id.' Password: '.$password;
        ...
        if ($admin->role_id == 4) {
            $admin->subjects()->syncWithoutDetaching($request->subject_id);
        }
    ...
    public function offline_student_admit_pdf($id)
    {
        $student=User::find($id);
        $student_subjects = StudentSubject::where('student_id', $student->id)->where('status', 1)->get();
        
        $pdf = PDF::loadView('admin.offline.pdfAdmit', compact('student','student_subjects'));
        // $pdf->setPaper('A4', 'landscape');
