   ...
   <header class="page-header">
					...
					<section class="panel">
							...
							<div class="panel-body">
								<form class="" action="{{action('SubjectController@update',$subject->id)}}" method="post">
									@csrf
									<input type="hidden" name="_method" value="PUT">
									<div class="form-group">
										<label class="col-md-2 control-label" for="inputDefault">Name</label>
											<div class="col-md-6">
												<input type="text" class="form-control" name="name" id="inputDefault" value="{{$subject->name}}">
											</div>
									</div>
									...
									<div class="form-group">
										...
										<div class="col-md-6">
											<select class="form-control mb-md" name="branch_id" id="branch_id">
												<option value="{{$subject->branch_id}}" selected="true">{{$subject->branch->name}}</option>
									...
									<div class="form-group">
										...
										<div class="col-md-6">
											<select class="form-control mb-md" name="class_id" id="class_id">
												<option value="{{$subject->class_id}}" selected="true">{{$subject->classname->name}}</option>
											...
											<select class="form-control mb-md" name="admin_id" id="admin_id" disabled>
												@foreach($admins as $admin)
												  	<option value="{{ $admin->id }}" 
														{{ ($subject->admins->first() && $subject->admins->first()->id == $admin->id) ? 'selected' : '' }}>
                    									{{ $admin->name }}
													</option>	
												@endforeach
												
											</select>
						 					<input type="hidden" name="admin_id" value="{{ $subject->admins->first() ? $subject->admins->first()->id : '' }}">
										</div>
									</div>
									<div class="form-group">
										<label class="col-md-2 control-label" for="inputDefault">Amount</label>
										<div class="col-md-6">
											<input type="number" step="0.01" name="amount" class="form-control" id="inputDefault" value="{{$subject->amount}}">
										</div>
									</div>
									...
									<div class="form-group">
										...
										<div class="col-md-6">
											<select class="form-control mb-md" name="student_type" id="student_type">
												<option value="0" {{ $subject->student_type == 0 ? 'selected' : '' }}>Offline</option>
												<option value="1" {{ $subject->student_type == 1 ? 'selected' : '' }}>Online</option>
